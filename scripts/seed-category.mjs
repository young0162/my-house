...
const seedOneFile = async (tx, filePath) => {
  ...
  const result = { filePath, parsed: items.length, created: 0, reused: 0, updated: 0 };
  ...
  for (const item of items) {
    ...
    const existing = await tx.category.findFirst({ where: { name: item.name, parentId } });

    if (existing === null) {
      const category = await tx.category.create({
        data: { name: item.name, slug: null, depth, sortOrder, isActive: true, parentId },
      });
      ...
      result.reused += 1;
      if (existing.depth !== depth || existing.sortOrder !== sortOrder || existing.isActive !== true) {
        const category = await tx.category.update({
          where: { id: existing.id },
          data: { depth, sortOrder, isActive: true },
        });
    ...
    JSON.stringify(
      {
        before,
        after,
        totalParsed: results.reduce((sum, r) => sum + r.parsed, 0),
        totalCreated: results.reduce((sum, r) => sum + r.created, 0),
        totalReused: results.reduce((sum, r) => sum + r.reused, 0),
